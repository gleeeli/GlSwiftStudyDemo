...
public extension NSAttributedString {
    ...
    static func getNormalAttribute(content: String, font: UIFont, textColor: UIColor?) -> NSMutableAttributedString {
        ...
        let allR = NSRange(location: 0, length: content.count)
        let attributedString = NSMutableAttributedString.init(string: allText)

        attributedString.addAttributes([NSAttributedString.Key.foregroundColor: textColor ?? UIColor.black], range: allR)
    ...
    static func getAdjustFont(maxWidth: CGFloat, muattribute: NSMutableAttributedString, maxLines: Int, fontSize: CGFloat ) -> CGFloat {
        ...
        muattribute.addAttributes([NSAttributedString.Key.font: UIFont.systemFont(ofSize: fontSize)], range: NSRange(location: 0, length: muattribute.length))
