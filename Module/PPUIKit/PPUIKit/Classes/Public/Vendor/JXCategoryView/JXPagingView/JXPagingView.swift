...
open class JXPagingView: UIView {
    ...
    private weak var jxdelegate: JXPagingViewDelegate?
    ...
    public init(delegate: JXPagingViewDelegate, listContainerType: JXPagingListContainerType = .collectionView) {
        self.jxdelegate = delegate
        self.listContainerType = listContainerType
        super.init(frame: CGRect.zero)

        listContainerView.jxdelegate = self
        ...
        mainTableView.isDirectionalLockEnabled = true
    ...
    open func checkDelegates(jxdelegates: [JXPagingViewListViewDelegate]) {
        var index = 0
        jxdelegates.forEach { jxDelegate in
            if validListDict.keys.contains(index) == false {
                jxDelegate.listViewDidScrollCallback {[weak self, weak jxDelegate] (scrollView) in
                    self?.currentList = jxDelegate
                    self?.listViewDidScroll(scrollView: scrollView)
                   }
                validListDict[index] = jxDelegate
                mainTableView.reloadData()
            }
            index += 1
        }
    ...
    open func resizeTableHeaderViewHeight(animatable: Bool = false, duration: TimeInterval = 0.25, curve: UIView.AnimationCurve = .linear) {
        guard jxdelegate != nil else { return }
        if animatable {
            ...
            bounds.size.height = CGFloat(jxdelegate?.tableHeaderViewHeight(in: self) ?? 0)
    ...
    func refreshTableHeaderView() {
        guard jxdelegate != nil else { return }
        let tableHeaderView = jxdelegate?.tableHeaderView(in: self)
        let containerView = UIView(frame: CGRect(x: 0, y: 0, width: 0, height: CGFloat(jxdelegate?.tableHeaderViewHeight(in: self) ?? 0)))
        containerView.addSubview(tableHeaderView ?? UIView())
    ...
    func mainTableViewMaxContentOffsetY() -> CGFloat {
        guard jxdelegate != nil else { return 0 }
        return CGFloat(jxdelegate?.tableHeaderViewHeight(in: self) ?? 0) - CGFloat(pinSectionHeaderVerticalOffset)
    }
    ...
    func pinSectionHeaderHeight() -> CGFloat {
        guard jxdelegate != nil else { return 0 }
        return CGFloat(jxdelegate?.heightForPinSectionHeader(in: self) ?? 0)
    }
...
extension JXPagingView: UITableViewDataSource, UITableViewDelegate {
    ...
    open func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        guard jxdelegate != nil else { return nil }
        return jxdelegate?.viewForPinSectionHeader(in: self)
    }
    ...
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        jxdelegate?.mainTableViewDidScroll?(scrollView)
        jxdelegate?.pagingView?(self, mainTableViewDidScroll: scrollView)
    }

    open func scrollViewWillBeginDragging(_ scrollView: UIScrollView) {
        // 用户正在上下滚动的时候，就不允许左右滚动
        listContainerView.scrollView.isScrollEnabled = false
        jxdelegate?.pagingView?(self, mainTableViewWillBeginDragging: scrollView)
    }
    ...
    open func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        jxdelegate?.pagingView?(self, mainTableViewDidEndDragging: scrollView, willDecelerate: decelerate)
    }
    ...
    open func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        ...
        jxdelegate?.pagingView?(self, mainTableViewDidEndDecelerating: scrollView)
    }
    ...
    open func scrollViewDidEndScrollingAnimation(_ scrollView: UIScrollView) {
        ...
        jxdelegate?.pagingView?(self, mainTableViewDidEndScrollingAnimation: scrollView)
    }
}
...
extension JXPagingView: JXPagingListContainerViewDataSource {
    public func numberOfLists(in listContainerView: JXPagingListContainerView) -> Int {
        guard jxdelegate != nil else { return 0 }
        return jxdelegate?.numberOfLists(in: self) ?? 0
    }
    ...
    public func listContainerView(_ listContainerView: JXPagingListContainerView, initListAt index: Int) -> JXPagingViewListViewDelegate {
        guard jxdelegate != nil else { fatalError("JXPaingView.delegate must not be nil") }
        var list = validListDict[index]
        if list == nil {
            list = jxdelegate?.pagingView(self, initListAtIndex: index)
    ...
    public func scrollViewClass(in listContainerView: JXPagingListContainerView) -> AnyClass {
        if let any = jxdelegate?.scrollViewClassInListContainerView?(in: self) {
            return any
        }
        return UIView.self
    }
