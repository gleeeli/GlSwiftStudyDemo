...
public enum PPCommPageControlAliment {
    case Center
    case Left
    case Right
}

public protocol PPCommPageControlDelegate {
    func pageControlClick(pageControl: PPCommPageControl, index: Int)
}

public class PPCommPageControl: UIControl {
    
    /// 闭包点击事件
    public var clickPointClosure: ((_ num : Int) -> Void)?

    /// 当前点的大小
    public var currentPointSize: CGSize = CGSize(width: 6, height: 6) {
        didSet{
    ...
    /// 其他点的大小
    public var otherPointSize: CGSize = CGSize(width: 6, height: 6) {
        didSet{
    ...
    /// 点-切圆角
    public var pointCornerRadius:CGFloat = 3 {
        didSet{
            if numberOfPages > 0 {
                createPointView()
            }
    ...
    /// 位置
    public var pageAliment: PPCommPageControlAliment = .Center {
        didSet{
            if numberOfPages > 0 {
                createPointView()
            }
    ...
    ///左右间距
    public var leftAndRightSpacing: CGFloat = 10 {
        didSet{
    ...
    /// 其他点未选中颜色
    public var otherColor: UIColor = .gray {
        didSet{
            if numberOfPages > 0 {
                createPointView()
            }
    ...
    /// 当前点颜色
    public var currentColor: UIColor = .orange {
        didSet{
            if numberOfPages > 0 {
                createPointView()
            }
    ...
    /// 其他点背景图片
    public var otherBkImage: UIImage? {
        didSet{
            if numberOfPages > 0 {
                createPointView()
            }
    ...
    /// 当前点背景图片
    public var currentBkImage: UIImage? {
        didSet{
            if numberOfPages > 0 {
                createPointView()
            }
    ...
    ///当前选中点的layer宽
    public var currentLayerBorderWidth: CGFloat = 1 {
        didSet{
            if numberOfPages > 0 {
                createPointView()
            }
    ...
    ///其他点的layer宽
    public var otherLayerBorderWidth: CGFloat = 1 {
        didSet{
            if numberOfPages > 0 {
                createPointView()
            }
    ...
    ///当前选中点的layer颜色
    public var currentLayerBorderColor: UIColor? {
        didSet{
            if numberOfPages > 0 {
                createPointView()
            }
    ...
    ///其他选中点的layer颜色
    public var otherLayerBorderColor: UIColor? {
        didSet{
            if numberOfPages > 0 {
                createPointView()
            }
    ...
    /// 当只有一个点的时候是否隐藏
    public var isHidesForSinglePage = false {
        didSet{
            if numberOfPages > 0 {
                createPointView()
            }
    ...
    /// 是否可以点击 默认不可以点击
    public var isCanClickPoint = false {
        didSet{
            if numberOfPages > 0 {
                createPointView()
            }
    ...
    /// 协议代理
    var pageControlDelegate: PPCommPageControlDelegate?
        
    private var dots = [UIImageView]()
    
    /// 创建视图
    public func createPointView() {
        /// 先清除视图
        clearView()
    ...
    @objc private func clickAction(_ recognizer: UITapGestureRecognizer) {
        ...
        if let tag = recognizer.view?.tag {
            ...
            if index >= 0 {
                currentPage = index
                pageControlDelegate?.pageControlClick(pageControl: self, index: index)
